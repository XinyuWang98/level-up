...
> 🎯 对应简历项目：**Story 1 — 用户行为轨迹 XGBoost 预测流失意图** | **隐藏武器 — 准时宝 K-Means 分群**
...
!!! info "AUC-ROC vs Precision-Recall"
    **AUC-ROC**：适用于正负样本比较均衡的场景（如流失 vs 不流失，比例不太极端时）。
    **Precision-Recall**：适用于**正样本极少**的场景（如欺诈检测、罕见疾病）。
    **面试话术**："在我的流失预测项目中，正负比约 3:7，我选 AUC 作为主指标。如果正负比到 1:100，我会转用 PR-AUC。"
...
!!! info "SHAP 模型解释"
    回答"为什么他会流失/流失"：SHAP 把每个特征对单个预测的贡献量化为一个值。
...
??? warning "Q5: 用户行为轨迹 标注策略——用户没点流失但骂完走了，算正样本吗？"
    **来源**：Gemini 模拟面试 Q1
...
??? warning "Q7: 闭环落地——具体举一个知识库盲区修复的例子？ ⭐"
    **来源**：DeepSeek 模拟面试 Q2
    **答法**：必须讲 STAR 闭环 → 发现问题（退货政策关键词高频命中但满意度低）→ 根因（描述拗口）→ 解决（优化话术）→ 验证（A/B 测试核心流失率优化 X%）
