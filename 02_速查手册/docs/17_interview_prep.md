...
??? example "Story 1: A/B 实验平台体系建设 - 分析周期 7->3 天 :star::star::star:"
    ...
    **可能的追问 & 应对**：
    ...
    - *"流量冲突怎么处理？"* - [查看回答策略](#难题-4分层实验框架如何避免流量污染)
...
??? example "Story 2：用户体验诊断体系 (VoC) → 转人工率 -9pp ⭐⭐⭐"
    ...
    **可能的追问 & 应对**：
    ...
    - *"9pp 如何归因？"* → [查看回答策略](#难题-1转人工率下降-9pp如何归因到你的项目因果推断)
...
??? note "Q2: XGBoost 预测转人工意图，做了哪些特征工程？定位了哪些知识库盲区？"
    **考察点**：特征工程思路、模型可解释性、业务落地。
    ...
    → [查看回答策略：XGBoost 特征重要性解读](#难题-3xgboost-特征重要性如何解读)

??? note "Q3: 转人工率下降 9pp，如何归因？有没有做因果推断？ ⭐必练"
    **考察点**：因果推断敏感度，科学评估项目效果。
    
    → [查看回答策略：因果归因](#难题-1转人工率下降-9pp如何归因到你的项目因果推断)
...
??? note "Q7: 分层重叠实验框架如何正交分流？流量冲突怎么处理？ ⭐必练"
    **考察点**：分层实验架构、流量分割、互斥实验。
    ...
    → [查看回答策略：分层框架避免流量污染](#难题-4分层实验框架如何避免流量污染)
...
??? note "Case 2: 如何构建商家健康度评分模型？ ⭐必练"
    ...
    → [查看回答策略：搭建商家服务体验指标体系](#难题-2如何搭建商家服务体验指标体系)
...
??? note "Q4: 从消费者侧转商家侧，最大挑战是什么？"
    → [查看回答策略：搭建商家指标体系](#难题-2如何搭建商家服务体验指标体系)
...
|   3   | **SQL/Python 手撕代码**        | 窗口函数、Pandas 多表 merge、简单建模 → 参考 [SQL 速查手册](11_sql.md)    |   ⭐⭐   |
...
!!! info "规则"
    状态为 `✅ 已投递` 的简历**自动封板**，禁止任何后续编辑。详见 [简历生成 Workflow](../../.agent/workflows/generate_resume.md)。
