...
|  ⭐⭐⭐   | [项目深挖题](#21-项目经历深挖必问)   | 个性化推荐/核心系统 / 人力调度 / A/B 平台 10 题 |  🔲 待练习  |
...
> **递进逻辑**：打地基（埋点）→ 发现问题（用户行为轨迹）→ 预测优化（人力）→ 量化评估（实验平台）

| 项目                    | 时间段            |  周期   | 面试策略                               |
| :---------------------- | :---------------- | :-----: | :------------------------------------- |
| 🏗️ 埋点治理与数仓建设    | 2022 Q4 - 2023 Q2 | ~6个月  | 提一句即可，面试官追问才展开           |
| 🔍 用户行为轨迹 体验诊断 | 2023 Q2 - 2024 Q2 | ~12个月 | ⭐ **主讲项目之一**，9pp 归因拆分是亮点 |
| 📊 人力预测              | 2024 Q1 - 2024 Q3 | ~6个月  | 提一句即可，与 用户行为轨迹 有部分并行 |
| 🧪 A/B 实验平台          | 2024 Q4 - 2025 Q2 | ~6个月  | ⭐ **主讲项目之一**，技术深度最硬       |

!!! warning "数字锁定（所有回答必须统一）"
    - 实验平台：周期缩短 60%（25→10天）、规避 5 次负向上线、智能话术降低 3.2pp
    - 用户行为轨迹 诊断：核心流失率 37%→28%（-9pp），诊断体系独立贡献 6.9pp（77%）
    - 人力预测：MAPE ≤ 8%、人力利用率提升 12%

??? example "Story 1: A/B 实验平台体系建设 - 分析周期 7->3 天 :star::star::star:"
    |     STAR     | 内容                                                                                                                                                                                                                                                                                                           |
    | :----------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | **S** (背景) | 商品推荐策略迭代缺乏量化评估，核心指标"核心放弃率"存在浏览 (Session) 和用户 (User) 两个分析粒度。浏览级是 Binary 指标 (0/1)，用户级因访问次数不同退化为 Ratio Metric（放弃次数/总访问数）。实验分析周期长达 7 天+，严重拖慢策略迭代节奏                                                                        |
    | **T** (任务) | 0-1 主导搭建分层重叠实验框架，构建 DTD（Day-To-Day 每日切片）和 LTD（Life-To-Date 累计至今）双轨监控体系，将分析周期从 7 天压缩到 3 天                                                                                                                                                                         |
    | **A** (行动) | (1) Hash 取模实现用户层/会话层/策略层正交分流 + 互斥域隔离 (2) 对用户级 Ratio Metric 采用 **Delta Method** 校正方差 (3) 引入 **CUPED** 利用实验前历史访问频次和放弃频次作为协变量，分别对分子分母做方差缩减（~30%+）(4) 为 LTD 曲线引入 **mSPRT 贯序检验**，基于 $1/\alpha$ 似然比阈值实现安全的中途决策与早停 |
    | **R** (结果) | 分析周期从 **7 天缩短到 3 天**，策略上线周期压缩 60%。"智能留存弹窗"实验在 3 天内检测出核心放弃率降低 **3.2pp** 的显著结论。通过严谨检验框架规避了 5 次负向策略上线                                                                                                                                            |
    ...
    **可能的追问 & 应对**：
    ...
    - *"核心放弃率是 Ratio Metric，怎么做 CUPED？"* - 不直接对比值做 CUPED。拆分成子（放弃次数）和分母（总访问数），分别用历史数据做 CUPED 方差缩减，然后用 Delta Method 融合计算最终显著性
    - *"流量冲突怎么处理？"* - [查看回答策略](#p7-4)
    - *"SRM 常见原因？"* - 爬虫流量、触发条件 Bug、分桶逻辑错误

    **🔍 “放弃/取舍”类深挖防御**：

    - *"分层实验中你放弃了哪些分流方案？为什么？"* → "最初考虑过基于 Cookie 的分流，但推荐场景下浏览器晚告都会刷新 Cookie，导致同一用户被重复分流。最终选用 User ID Hash 取模，因为它能确保同一用户在实验期间始终在同一组。"
    - *"CUPED 的协变量选择上，你**筛掉**了哪些候选变量？标准是什么？"* → "最初尝试过用2周前的 DAU 作为协变量，但发现它与“核心放弃率”的相关性只有 0.15，方差缩减不到 5%。必须用**同指标的历史值**（实验前 7 天的访问频次和放弃频次），相关性超过 0.6，方差缩减才达到 30%+。**标准就是 Cov(X,Y)/Var(X) 中的 θ 足够大。**"
    - *"有没有一个实验结果显著但你建议**不上线**的 case？为什么？"* → "有。有一次我们的“强制全屏导流”策略 P=0.02，核心放弃率下降了 1.5pp，但护栏指标显示用户“短时重复跳出率”上升 3pp——说明用户并没有真正产生意向转化，只是被“强制打断”后关闭再重新点开而已。所以我建议不上线。**核心沟通：“主指标显著但护栏指标崩了 = 捡了芝麻丢了西瓜”。**"
    ...
    **第三层（高阶实战痛点 - 如何解决稀释效应）【绝杀核心】：**
    “更深的痛点是，这种架构极易引发**稀释效应（Dilution Effect）**。比如我们在 Layer 3 做了一个仅针对‘商企用户特卖’的极度细分优化。因为工程性能受限，没法做到强实时的『按需触发式分流』，所以我从数据流和指标口径上下手。我要求工程团队在所有分层实验中，不论层级多深，只要命中策略曝光，必须上报一个统一的 **Exposure 打点**。然后在我的评估自动化体系里，不再单纯拿整个 Layer 分配到的 ITT（意向群体）去算 P 值，而是支持基于 Exposure 触发点，去计算 **LATE（局部平均因果效应）**。通过过滤掉没曝光的基线噪音，最终成功找回了多个原本被误判为无效的高价值策略。”
    ...
    *来源：豆包模拟面试整体评价——"对核心数据无肌肉记忆，资深候选人需张口就来"*
    ...
    | 触达率（Dilution 严重度） | ~2%（特定长尾品类）        | "10 万访问中仅约 2000 人触发该品类卡片，触达率仅 2%"    |
    | CUPED 方差缩减            | 30%+                       | "用实验前 14 天历史曝光频次做协变量，方差缩减 30%+"     |
    | 分析周期压缩              | 7 天 → 3 天                | "结合 CUPED + mSPRT，分析周期从 7 天压缩到 3 天"        |
    | 策略上线周期压缩          | 60%                        | "策略上线周期整体压缩了 60%"                            |
    | 智能留存弹窗效果          | 核心放弃率降低 3.2pp       | "3 天内检测出核心放弃率降低 3.2pp，P < 0.01"            |
    ...
    **核心话术**：
    ...
    1. **历史基线提取**：拉取过去 90 天的护栏指标（如页面跳出率、用户退单率）日级数据，计算均值和标准差
...
??? example "Story 2：用户体验诊断体系 (用户行为轨迹) → 核心流失率 -9pp ⭐⭐⭐"

    > ⚠️ **面试策略：主打"标签→定位→异常诊断→产品化"四步闭环叙事。被追问技术细节时转向"DA 做的是交叉分析和异常检测，不是训练模型"。**

    |     STAR     | 内容                                                                                                                                                                                                                                                                      |
    | :----------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
    | **S** (背景) | 日均浏览 2M+，核心购买漏斗流失率 37%，不同市场转化率差异大但问题根因不清晰                                                                                                                                                                                                |
    | **T** (任务) | 建立用户跳出体验诊断闭环，规模化定位痛点并推动修复                                                                                                                                                                                                                        |
    | **A** (行动) | ① 建标签体系：给流量打"是否跳出/是否短停"标签 → ② **问题定位**：Python 提取跳出前高频搜索词 + 算法团队页面分类标签做交叉聚合 → ③ **YoY 异常诊断**：与去年同期对比，聚焦今年显著恶化或新涌现的问题 → ④ **产品化交付**：Tableau 实时看板 + 企微推送 TOP100 异常流失链路明细 |
    | **R** (结果) | 累计识别 Top 30 引导体验痛点，推动修复后核心流失率下降 **9pp**，方法论沉淀为团队 SOP                                                                                                                                                                                      |

    **可能的追问 & 应对**：

    - *"9pp 如何归因？"* → [查看回答策略](#p7-1)
    - *"既然有算法团队的主题标签了，为什么还要拆高频词？"* → "主题标签是粗粒度的，比如'发货时效'。但用户到底是因为'物流追踪未更新'还是'包裹预售未提示'不满意跳出？高频词提供的是主题下面的**细粒度根因**。而且新涌现的问题主题模型可能还没覆盖到，高频词是一个自下而上的补充信号。"
    - *"高频搜索词里有很多没意义的词，怎么过滤？"* → "三层过滤：第一层通用停用词；第二层业务停用词（我们在电商推荐场景积累的词表，比如'订单号'出现频率高但没有诊断价值）；第三层我只看流失链路的高频搜索词与全量大盘搜索词的**差异词**——一个词在流失组里高频但在整体里不高频，才值得关注。"
    - *"你做的 NLP 是什么级别的？"* → "我做的不是算法侧的深层 NLP 建模。核心工作是诊断闭环——从转化指标下钻到搜索词明细确认根因，再验证普遍性、推动修复。文本分析工具是我**规模化发现问题**的手段。更深的模型能力（意图识别等）是算法团队负责的，我负责定义异常阈值、验证策略效果、把产出转化为业务建议。"
    - *"交叉聚合具体怎么做的？"* → "简单来说就是 group by 主题标签，然后在每个主题内部看高频搜索词/意图表现。比如'退换货'主题下，高频词如果是'表单复杂'、'找不到入口'，就说明问题不是政策本身，而是产品引导不清晰——这直接指向了前端 UI 优化。"

    **🔍 “放弃/取舍”类深挖防御**：

    - *"高频词筛选中你**放弃了哪些**看起来重要但实际没用的词？"* → "比如'我的订单'这个词，在电商平台它在所有访问轨迹里都高频出现，但没有任何异常诊断价值。还有'首页'粗粒度主题词也被我归入了业务停用字典，因为它不帮你定位'到底哪里痛'。**筛选标准：一个词在流失人群中高频但在全量人群中不高频，才有诊断价值。**"
    - *"Top 30 痛点中，有没有你**明确不推动修复**的？为什么？"* → "有。比如'衣服尺码偏差'这个痛点排名前 5，但我没有推动平台侧强制拦截，因为根因在商家的供应链侧（工厂生产标准不统一），平台层能做的只是强弹提示用户查看尺码表。我把这个 insight 同步给了商家治理团队，但短期内推荐侧没有彻底解决的手段。**关键认知：不是所有痛点都在你的能力圈内，要学会分类处理。**"
    - *"你的异常标记方案**迭代过几版**？初始版本有什么问题被你推翻了？"* → "至少迭代了 3 版。初始版只用'是否退出 App'作为标签，但发现漏掉了'反复搜索未果直接切店'的用户。第二版加入了停留极短阈值，但引入了一批'秒开秒买明确意图'的优质用户的误报。第三版最终稳定为多维策略：无结果返回 + 短停跳出 + 会话异常中止（session drop）三个信号至少命中两个。**数据团队每周会对边缘 case 做人工校准抽取。**"

??? example "Story 3：大促仓储履约人力调度预测优化 → MAPE ≤ 8% ⭐⭐⭐"

    > ⚠️ **面试策略：主打"多层漏斗拆解 + 人机协同"叙事，被追问模型时转向"业务特征设计更重要"。**

    |     STAR     | 内容                                                                                                                                                                                                                                          |
    | :----------: | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | **S** (背景) | 仓储拣货与打包团队排班依赖人工经验，多时区订单大促峰值叠加导致人力缺口 40%                                                                                                                                                                    |
    | **T** (任务) | 设计端到端预测体系，从流量预估到打包履约排班全链路打通                                                                                                                                                                                        |
    | **A** (行动) | ① 基于运营侧大盘单量预估，按各仓历史吞吐比拆解分拣量与打包量 → ② 结合各环节 TPH (每小时单量) 和 HC (人数) 数据推算月度人力需求 → ③ 按订单流入小时波峰分布拆解动态排班 → ④ 仓储负责人确认（允许 ±10%微调）→ ⑤ A/B 实验验证 AI 调度 vs 人工排班 |
    | **R** (结果) | 预测 MAPE ≤ **8%**，峰值订单滞压控制在积压警戒线内，人力资源利用率提升 **12%**                                                                                                                                                                |

    **可能的追问 & 应对**：

    - *"模型用的什么？"* → "这个项目的核心不是某个模型，是整个预测漏斗的设计。Prophet 做趋势分解是其中一步，更关键的是业务逻辑拆解——从总单量到各仓分拣量再到打包排班，每一步都有对应的吞吐转化模型。"
    - *"MAPE 8% 怎么做到的？"* → "最大提升不是调参，是拿到运营侧的宏观单量预估作为输入——这一步直接把误差缩了一半，因为履约吞吐压力本质上就是入口订单量的函数。"
    - *"人工微调±10%不会破坏模型吗？"* → "这恰恰是设计亮点。预测是建议而不是命令，仓储负责人有当地经验（比如知道某个节日只影响特定片区的运力），微调让他们有 ownership，落地阻力小很多。"
    - *"为什么用 Prophet 而不是 ARIMA？"* → "Prophet 自动处理趋势变化点和节假日效应，可解释性强——业务团队能直接看到趋势和季节性的分解图，沟通成本低。"

    **🔍 “放弃/取舍”类深挖防御**：

    - *"为什么**不用** ARIMA/XGBoost 做预测？你评估过后放弃的原因？"* → "评估过。**放弃 ARIMA** 因为大促履约数据是多仓多地域的，ARIMA 每个仓都要单独建模，参数调试成本太高。Prophet 一套 API 全市场通用，建模效率高 10 倍。**放弃纯 XGBoost** 因为它不能自动捕捉趋势和周期性，必须手动构造 lag/rolling 特征，稳定性不如 Prophet。但我保留了它作为残差修正层。"
    - *"仓储负责人的微调和你的预测**冲突**时怎么处理？"* → "实际发生过。东南亚某个仓的主管每次坚持额外加派人手，因为当地每月底有个本地物流商的截单节点我模型没覆盖到。我的处理：先让步，然后回馈实际积压数据，下个周期把这个节点加入模型的自定义周期参数。**模型是建议而不是命令，微调让业务有 ownership。**"
    - *"有没有某个仓的预测你**永远做不准**？怎么跟业务交代？"* → "有。新布点的小型前置仓，历史数据不到 3 个月，模型没有足够季节性信息。我跟业务说：'这个区域 MAPE 25%+，建议按风险缓冲额外加 15% 的机动人手池，同时用相似成熟仓的周期作为趋势参照。'**承认模型局限性比硬吹精度更专业。**"
...
> 针对 [你的前司名称] 的个性化推荐/核心系统、人力调度、A/B 实验平台三个重点项目，面试官会层层深入。

**个性化推荐/核心系统体验优化与流失归因**

??? note "Q1: 用户行为轨迹 的具体流程？用了哪些文本挖掘技术？如何处理多语言？"
    **考察点**：文本挖掘 Pipeline 熟悉度，多语言/非结构化数据挑战。
    
    **回答骨架**：分语种预处理 → 统一 Embedding → 关键词提取 (TF-IDF/TextRank) → 情感分析 → 聚类 → 人工校验 Top Cluster

??? note "Q2: XGBoost 预测流失意图，做了哪些特征工程？定位了哪些知识库盲区？"
    **考察点**：特征工程思路、模型可解释性、业务落地。
    
    → [查看回答策略：XGBoost 特征重要性解读](#p7-3)

??? note "Q3: 核心流失率下降 9pp，如何归因？有没有做因果推断？ ⭐必练"
...
??? note "Q9: 分析周期 7->3 天，用了什么加速方法？ :star:必练"
    ...
    **回答骨架（三层加速方案）**：
    ...
    1. **Delta Method 校正方差**：用户级核心转化率是 Ratio Metric（分母代表总访问/曝光且带随机波动），用 Delta Method 一阶泰勒展开精确估算方差
    2. **CUPED 方差缩减**：利用用户实验前两周的历史访客行为天数和历史互动频次作为协变量，分别对分子分母做 CUPED 修正，剥离用户固有行为差异，方差缩减 30%+
    ...
    **关键区分**：DTD（每日独立数据）可用硬编码 a=0.05（数据互不重叠）；LTD（累积数据）必须用 Sequential Testing（数据层层累加，偷看会导致假阳性膨胀）
    ...
    > **专家级防守话术 (三层连击)**：
    > 1. **业务场景解绑**：“质疑非常专业，大盘确实要跑满 14 天。但我们是**短链路页面弹窗/推荐卡片**场景。UI 层面的漏斗交互带有**强即刻反馈**属性，用户看到新版本卡片后 30 秒内就决定了是点击还是划走，这极其不同于长周期的延迟转化效应（如品牌心智建立）。因此只要单日流量并发量足够巨大，3 天的累积验证样本量对于捕获这种即刻心智行为是绰绰有余的。”
    > 2. **明确 mSPRT 的定位**：“其实这 3 天，更多是利用 mSPRT **对强负向/强正向策略的提前止损**。对于那些一上线就导致核心跳出率飙升的灾难性 Bug 策略，mSPRT 允许我们在第 3 天就合规击穿早期更为苛刻的门槛并执行下线，而不是非要硬扛着亏损跑满 7 天。这是保护业务的容错红线，而非为了几 bps 微弱涨跌的硬核显著性判定。”
    > 3. **CUPED 降维打击**：“更深层的底气来源于 **CUPED 方差缩减**。电商漏斗指标本底噪极大，我用 14 天的大盘历史行为数据（大基数协变量）硬生生压制了 30% 的核心离散度。方差的锐减导致我们探测相同 MDE(最小可检测效应) 所需的样本量同比例剧减，过去硬攒 7 天才能抵消冲平的随机噪音，现在结合 CUPED 后攒 3 天的纯度就足够看清因果增量了。”
...
??? note "Case 1: 巴西市场投诉率上升，如何定位核心痛点？"
    考察点：用户行为轨迹 方法论迁移能力、多源数据融合、根因分析框架。
    ...
    **5 步法骨架**：
    ...
    4. **Data Ask**：用户 App 内行为轨迹流 + 订单关联物流状态 + 舆情客诉标签
...
??? note "Q1: 对 [目标业务/产品] 有什么了解？主要市场？与[竞品/其他模式]的痛点差异？"
    **回答骨架**：主要市场（欧洲/东南亚/拉美/中东）→ 差异（多渠道物流时效长、本地化支付方式碎片化、多语言搜索意图差异、退换货逆向物流成本高、合规及关税挑战）

??? note "Q2: 电商/O2O商家最关心什么？如何用数据降低经营成本？"
...
<a id="p7-1"></a>
??? success "难题 1：核心流失率下降 9pp，如何归因到你的项目？（因果推断）"
    **考点**：对因果推断的敏感度，如何在非严格 A/B 实验下科学评估。
    
    **回答策略 (专家级话术)**：
    > "我们在项目上线时**刻意做了 phased rollout**（分阶段上线）。前两周只在部分长尾品类频道内灰度，用**同期的大盘基准对照组**（其余未灰度的核心品类频道）做对比。
    > 虽然这不是严格的随机对象实验，但我们针对自然渗透的用户用 **PSM 匹配**了同质化的画像特征（活跃频度、历史购买力基线、访问时段等），再用 **DID** 剔除了大盘自然的时间波动趋势。
    > 结果显示，灰度干预组的漏斗放弃率下降幅度显著高于对照组，且没有发生可观测的样本选择性偏移，我们才敢下结论归因。当然，为了达到最高严谨度闭环，随后我们在全量阶段又挂梯子在实验平台复测了一次准 A/B。"
...
??? success "难题 2：如何搭建商家服务体验指标体系？"
    ...
    **回答策略 (专家级话术)**：
    ...
    > 
    > 接着，参考我们在消费者侧的 用户行为轨迹 方法，对商家投诉/咨询文本做**关键词聚类**，定位高频痛点。再结合 **RFM 模型**的思路，对商家分层（头部/腰部/长尾），针对性设计体验优化策略。"
...
??? success "难题 3：XGBoost 特征重要性如何解读？"
    ...
    **回答策略 (专家级话术)**：
    ...
    > - **Gain (增益)**：看哪些特征对区分流失贡献最大（比如'对话轮次 > 5'）
...
|   2   | **梳理每个项目的因果推断细节** | 个性化推荐/核心系统：准备"没做因果推断会怎样误判"案例；人力调度：说明 A/B 实验可比性 |  ⭐⭐⭐   |
...
??? success "🏢 模板公司B — 模型策略分析师 (已沟通 YYYY-MM-DD)"
    ...
    **JD 适配分析示例：**
    ...
    | JD 要求          | 我的匹配点                        | 匹配度 |
    | :--------------- | :-------------------------------- | :----: |
    | 机器学习模型应用 | XGBoost 预测流失意图              |  ⭐⭐⭐⭐  |
    | 指标体系搭建     | 用户行为轨迹 体验诊断评估闭环搭建 | ⭐⭐⭐⭐⭐  |
