...
gantt
    ...
    section 泥土感重塑 (D1-D3)
    用户行为轨迹 x 因果推断缝合  :active, m1a, 2026-03-03, 1d
    排班 x AB高阶缝合   :m1b, 2026-03-04, 1d
    多渠道核武器词汇库    :m1c, 2026-03-05, 1d
    ...
    section 投递与面试
    密集投递 ([目标公司]+备选) :crit, d1, 2026-03-12, 3d
...
每个模块按照以下闭环循环推进，**每天额外搭配 1-2 道笔试编码题**（从 `05_笔试模拟` 目录取题）：
...
| **🎤 全仿真抗压测试**  | Day 8 - 14 | 用带有大促干扰、部门墙甩锅的跨界连环陷阱题压测，强制融入多渠道词库进行反击防守 | 坚不可摧的业务体感 |
...
#### 📅 3/3 周二 — 🛡️ 用户行为轨迹 业务线与因果推断 (PSM/DID) 缝合

|  时段  | 任务                                                                           | 配合资料                              |
| :----: | :----------------------------------------------------------------------------- | :------------------------------------ |
| 🌅 上午 | 撰写 500 字终极防御话术：证明核心流失率下降 9pp 是 用户行为轨迹 的真实独立贡献 | [因果面试题](17b_interview_causal.md) |
| 🌤 下午 | 提炼混淆变量（如：季节大促、同期 UI 变动），并准备语音口述模拟                 | [因果面试题](17b_interview_causal.md) |
| 🌙 晚上 | 💻 每天 30min SQL/Pandas 编码复健                                               | `05_笔试模拟/`                        |
...
#### 📅 3/5 周四 — 🌐 建立多渠道“核武器”词汇库

|  时段  | 任务                                                                                | 配合资料                            |
| :----: | :---------------------------------------------------------------------------------- | :---------------------------------- |
| 🌅 上午 | 梳理 10 个高频变数（支付熔断、供应链成本/渠道抽成博弈、大促爆仓、多语言情感极性等） | [业务分析](13_business_analysis.md) |
| 🌤 下午 | 将词汇库强制串联进至少 2 个相关的 STAR 项目经历中                                   | [业务分析](13_business_analysis.md) |
| 🌙 晚上 | 💻 每天 30min SQL/Pandas 编码复健                                                    | `05_笔试模拟/`                      |
...
|  时段  | 任务                                                             | 配合资料                            |
| :----: | :--------------------------------------------------------------- | :---------------------------------- |
| 🌅 上午 | 整理“商品曝光 -> 加购 -> 支付路由 -> 履约配送 -> 售后”核心漏斗图 | [业务分析](13_business_analysis.md) |
| 🌤 下午 | 构建“入驻 -> 流量变现 -> 履约 -> 纠纷 -> 流失”商家生命周期       | [业务分析](13_business_analysis.md) |
| 🌙 晚上 | 掌握商家经营成本（佣金/营销费/物流费）和降本破局点的数据拆解     | [业务分析](13_business_analysis.md) |
...
| 跨部门利益冲突（如拉新活动导致负面反馈激增），如何用数据定分止争？ | 数据归因的严谨度         |
