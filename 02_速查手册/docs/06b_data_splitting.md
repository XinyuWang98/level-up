...
for fold in range(N_SPLITS):
    ...
    print(f"Fold {fold+1}: Train {train_mask.sum()} | Test {test_mask.sum()} | M[目标业务] {mae:.2f}")
```
...
*你的直觉是对的！在 [前司名称]/TikTok 这种快节奏业务中，2年前的数据确实可能"过时"了。*
...
*   **适用**: 时尚电商 ([前司名称])、短视频推荐 (TikTok) — 潮流变化极快。
...
!!! tip "面试话术：Big Data Split"
    ...
    "在 [前司名称] 这种数据量级下，我们通常采用 **6:2:2** 的严格时序切分。同时为了应对 **Concept Drift**，我们会使用 **Sliding Window** 策略，或者在 Loss Function 中加入 **Time Decay**，确保模型对即时趋势保持敏感。"
